...
export class UpdateMeetDto extends CreateMeetDto {
  @IsArray({ message: MeetMessagesHelper.UPDATE_OBJECTNAME_NOT_VALID })
  @Type(() => UpdateMeetObjectDto) // Não entendi essa parte do type e call back.
  @ValidateNested({ each: true }) // ValidateNested faz a validação dos objetos abaixo.
  objects: Array<UpdateMeetObjectDto>;
}

export class UpdateMeetObjectDto {
  @IsNotEmpty({ message: MeetMessagesHelper.UPDATE_OBJECTNAME_NOT_VALID })
  name: string;
